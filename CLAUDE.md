...
- **`instantiate_task` no es idempotente, a diferencia de `complete_task`/`skip_task`.** Dos llamadas crean dos ocurrencias. La base no lo impide a propósito (crear dos veces la misma tarea es legítimo); quien ofrece un botón mira antes `v_today_tasks.template_id` para saber si ya hay una pendiente. Así lo hace la pantalla "Crear" del deck.
