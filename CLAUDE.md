...
- **El contrato está abierto a DOS roles, no a uno: `anon` y `authenticated`.** El daemon de la Pi entra con la clave publishable sola (rol `anon`); la PWA entra con Supabase Auth, y en cuanto hay sesión PostgREST resuelve el JWT y la petición corre como `authenticated`. El bloque 2 de `rls_contract.sql` revocó todo a los dos roles, pero el bloque 3 solo se lo devolvió a `anon` — de ahí `20260918120000_contrato_authenticated.sql`, que replica los mismos grants para `authenticated` sin revocar nada de `anon`. Consecuencia práctica: **un objeto nuevo del contrato necesita dos grants, no uno**, y olvidar el segundo no rompe el deck, rompe solo la PWA, que es justo el fallo que más tarda en verse. Cerrar `anon` es el final previsto, pero exige migrar antes el daemon a sesión autenticada.
...
- **El linter de Supabase da cinco tipos de aviso asumidos, y hay que distinguirlos de uno real.** `rls_enabled_no_policy` (INFO) en las 13 tablas — es literalmente el diseño: RLS activo y sin políticas es lo que las deja cerradas, y los clientes entran por vistas y RPC `security definer`; si a una tabla le apareciera una política, eso sí habría que mirarlo—, `security_definer_view` en las 8 vistas (es el mecanismo: sin él, `anon` no puede leer nada — "arreglarlo" con `security_invoker = true` las deja devolviendo cero filas y sin error), `function_search_path_mutable` en `app_today()`/`app_timezone()` (son `security invoker`, no escalan privilegios, y la cláusula `SET` les quitaría el inlining en los predicados de las vistas), y `anon_security_definer_function_executable` en las funciones `security definer` del contrato — es literalmente el contrato funcionando, `anon` debe poder llamarlas. El quinto es `authenticated_security_definer_function_executable`, sobre esas mismas funciones: el espejo exacto del anterior desde que el contrato se abrió también a `authenticated`, y se lee igual. Hoy son 10 en cada uno (las 12 del contrato menos `app_today`/`app_timezone`, que son `security invoker` y no salen). Si cualquiera de los dos apareciera en una función que NO está en la tabla de "Escritura" de `docs/contrato.md` (p.ej. `chain_next_occurrence`), eso sí es una fuga real: significa que su `grant` se coló donde no debía. Cualquier otro aviso hay que mirarlo.
...
- **Toda función nueva necesita su `grant execute` explícito a `anon` *y* a `authenticated`** si va a formar parte del contrato, y no llevarlo es lo correcto si no. Ver el punto sobre `PUBLIC` más arriba y el de los dos roles más abajo. Lo mismo vale para el `grant select` de una vista nueva.
